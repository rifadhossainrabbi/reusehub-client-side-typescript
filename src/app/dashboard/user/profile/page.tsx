...
import { User, Mail, Shield, Camera, Save, Edit2, Loader2, Link as LinkIcon } from 'lucide-react';
...
const UserProfile = () => {
  ...
  if (!user) return <div className="h-96 flex items-center justify-center animate-pulse text-blue-600 font-black uppercase tracking-widest">Identifying Seeker...</div>;
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <header className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Account Identity</h1>
          <p className="text-slate-500 font-medium italic">Manage your public presence in the ReuseHub network.</p>
        </div>
        <button 
        ...
        <div className="lg:col-span-5 bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col items-center text-center space-y-6">
          <div className="relative group">
            <div className="w-40 h-40 rounded-full overflow-hidden border-4 border-white dark:border-slate-800 shadow-2xl ring-2 ring-blue-500/20 bg-slate-50 dark:bg-slate-950 flex items-center justify-center">
               {user.image ? (
                 <img src={user.image} alt="p" className="w-full h-full object-cover transition-transform group-hover:scale-110" />
               ) : (
          ...
          <AnimatePresence mode="wait">
            {!isEditing ? (
              ...
              >
                ...
                <h4 className="text-xl font-bold text-slate-400">Identity Encrypted</h4>
                <p className="text-sm text-slate-500 max-w-[250px]">To change your information, click the edit button in the top right corner.</p>
              </motion.div>
            ) : (
              <motion.form 
              ...
              >
                <div className="space-y-6">
                   <div className="space-y-3">
                      <label className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] ml-1">Legal Name</label>
                      <input 
                   ...
                   <div className="space-y-3">
                      <label className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] ml-1">Avatar Artifact URL</label>
